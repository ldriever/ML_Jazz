'''
This file contain the function for filling the "beat", "bar" and "pitches" columns in the final output csv file.
...
# auxilliary function to create mask
def mask_first(x):
...
# Function for filling in the melody information into the dataframe
def fill_melody_info(df, save_path):
    '''
    INPUTS
        df              pandas dataframe containing the outputs of the ML model, with columns bar, beat, and pitch to be filled

        save_path       string specifying the file path to which the output csv is to be saved. IMPORTANT, save_path should end in ".csv"

    OUTPUT
        None, but saves filled csv file to save_path
    '''
    ...
    df.to_csv(current_path + "/" + save_path, index = False)
